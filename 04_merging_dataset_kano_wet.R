...
# data paths 
kanowet_household_list <- read_dta(file.path(dhsDir,"nigeria/kano_ibadan_epi/new_field_data/kano_malaria_weighted_information_final_wet_season_new.dta"))
...
#kano_household_list <- read.csv(file.path(cleaned_data_path, metropolis_name,"kano_malaria_weighted_information_dry_season.csv"))
 ...
 kn_household_net_insp <- read_dta(file.path(LuDir, "5. KN Wet season household net inspection.dta" ))
 kn_household_travelers <- read_dta(file.path(LuDir, "3. KN Wet season household travellers.dta" ))
 kn_household_visitors <- read_dta(file.path(LuDir, "2. KN Wet season household visitors.dta" ))
kn_women_survey <- read_dta(file.path(LuDir, "KN Wet season women survey_edited_290924.dta" ))
kn_womenmal_survey <- read_dta(file.path(LuDir, "KN Wet season women malaria hx_280924.dta" ))
kn_men_survey <- read_dta(file.path(LuDir, "KN Wet season men survey_edited 290924.dta" ))
kn_womenmob_survey <- read_dta(file.path(LuDir, "KN Wet season women mobility patterns_280924.dta" )) 
#

kn_women_survey <- left_join(kn_women_survey, kn_womenmal_survey,  by = c("sn") )


wide_kano_data <- kanowet_household_list %>%
  pivot_wider(
    names_from = hl1, 
...
wide_kano_datadetails <- kanowet_household_list %>%
  pivot_wider(
    names_from = hl1, 
    values_from = c(hl2:q304)
  ) %>%
...
widedata_hh <- left_join(wet, wide_kano_data, wide_, by ="sn") 

widedata_hh <- widedata_hh %>%  
  dplyr::select(-144:-145)
...
wide_kn_household_travelers <- kn_household_travelers %>%
  dplyr::select(-c(redcap_repeat_instrument)) %>% 
  pivot_wider(names_from = c(redcap_repeat_instance, c1iii ), 
...
widedata_hh$sn <- as.character(widedata_hh$sn)
kn_women_survey$sn <- as.character(kn_women_survey$sn)
wide_kn_household_net_insp$sn <- as.character(wide_kn_household_net_insp$sn)
wide_kn_household_travelers$sn <- as.character(wide_kn_household_travelers$sn)
wide_kn_household_visitors$sn <- as.character(wide_kn_household_visitors$sn)


sapply(list(widedata_hh, kn_men_survey, kn_women_survey, 
            wide_kn_household_net_insp, wide_kn_household_travelers, 
            wide_kn_household_visitors), function(df) class(df$sn))

kn_men_survey$sn <- as.character(as.character(kn_men_survey$sn)) # Ensuring conversion
kn_men_survey <- kn_men_survey[!is.na(kn_men_survey$sn), ] # Remove rows with invalid `sn`
...
# Convert `sn` to numeric and keep unique rows
datasets <- list(widedata_hh, kn_men_survey, kn_women_survey, 
                 wide_kn_household_net_insp, wide_kn_household_travelers, 
                 wide_kn_household_visitors)

datasets <- lapply(datasets, function(df) {
  df$sn <- as.numeric(df$sn)
  df <- df[!duplicated(df$sn), ]  # Keep unique rows based on `sn`
  return(df)
})
...
widedata_hh <- datasets[[1]]
kn_men_survey <- datasets[[2]]
kn_women_survey <- datasets[[3]]
wide_kn_household_net_insp <- datasets[[4]]
wide_kn_household_travelers <- datasets[[5]]
wide_kn_household_visitors <- datasets[[6]]
...
workingsn <- kn_women_survey |> filter(sn %in% wide_data_together$sn)
View(workingsn)
workingsn2 <- wide_data_together  |> filter(sn %in% kn_women_survey$sn)
notworkingsn <- kn_women_survey |> filter(!sn %in% wide_data_together$sn)
...
write_dta(wide_data_together, "/Users/macbookpro/Urban Malaria Proj Dropbox/urban_malaria/data/nigeria/kano_ibadan_epi/Combined Working Data/Kano/Wet Season Data/Wide Data/kano_wetseason_wide_data_final.dta")

write_csv(wide_data_together, "/Users/macbookpro/Urban Malaria Proj Dropbox/urban_malaria/data/nigeria/kano_ibadan_epi/Combined Working Data/Kano/Wet Season Data/Wide Data/kano_wetseason_wide_data_final.csv")
...
write_dta(long_data_together, "/Users/macbookpro/Urban Malaria Proj Dropbox/urban_malaria/data/nigeria/kano_ibadan_epi/Combined Working Data/Kano/Wet Season Data/Long Data/kano_wetseason_long_data.dta")

write_csv(long_data_together, "/Users/macbookpro/Urban Malaria Proj Dropbox/urban_malaria/data/nigeria/kano_ibadan_epi/Combined Working Data/Kano/Wet Season Data/Long Data/kano_wetseason_long_data.csv")
